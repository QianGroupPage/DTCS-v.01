...
env_total_row_re = r"\s*total\s*(-?[0-9.]+\s+)(-?[0-9.]+\s+)(-?[0-9.]+\s+)(-?[0-9.]+\s+)(-?[0-9.]+\s+)(-?[0-9.]+)"
...
class XPSTPCorrelator:
    ...
    def __init__(self, tp_file: str, init_temp: float, init_pressure: float,
            adsorption: List[int], desorption: List[int], init_constants: Optional[List[float]] = None,
    ...
    def _parse_tp_file(self) -> None:
        ...
        self.zpe = zpe
        self.init_corr_g = self._corr_g(self.init_temp, self.init_pressure, self.h[self.init_temp],
                self.s[self.init_temp])
    ...
    def _parse_env_definition(self, it, temp: float):
        ...
        if match is None or len(match.groups()) != num_defs_env_total_row:
            raise ValueError('Invalid tp file format: ' + str(num_defs_env_total_row)
