...
from dtcs.common.const import TIME, PRESSURE, TEMPERATURE
...
class BulkRxnSystem(RxnSystemABC):
    ...
    # TODO: Modify __init__ so that it only accepts relevant (Bulk) input.
    ...
    def terms(self) -> List[Term]:
        """Gives all turns in the system, including those generated by Rxns."""
        terms = self.by_subclass()[Term]
        # TODO: This spot is dangerous if self.reactions gets shuffled.
        for index, rxn in enumerate(self.reactions):
            # This replaces the k and k-reverse symbols in each term with
            #  ones that it is keeping track of.
            terms.extend([term.subs(const.k_map(index)) for term in rxn.to_terms()])
        return list(util.flat(terms))

    # TODO: Move Concs & Schedules to a conditions collection (?)
    @property
    ...
    def get_ode_expressions(self) -> Dict[str, sym.Expr]:
        """Return a dictionary of species name: expressions, where the
        expressions correspond to the derivative of the concentration of that
        species.

        The dictionary may not have entries for each species.
        """

        # Make an emtpy ODE list
        odes = defaultdict(lambda: sym.sympify(0))

        # Sum all terms for each symbol
        for term in self.terms:
            odes[term.symbol.name] += term.expression

        # Set the conc diffeqs
        for equation in self.conc_diffeqs:
            odes[equation.symbol.name] = equation.expression

        return dict(odes)

    def get_conc_expressions(self) -> Dict[str, sym.Expr]:
        """Return a dictionary of species name: expressions, where the
        expressions correspond to the concentration of that species.

        The dictionary may not have entries for each species.
        """
        concs = defaultdict(lambda: sym.sympify(0))

        for conc_eq in self.conc_eqs:
            concs[conc_eq.symbol.name] = conc_eq.expression

        return dict(concs)

    def get_concentration_function(self):
        """This function dictactes the behaviors of ConcEqs; It takes in
        concentrations, and outputs those same concentrations, except with
        species subject to a ConcEq have values according to it."""

        # This list is of the format {specie name: sympy expr} and may not
        #  cover self.species.
        conc_exs = self.get_conc_expressions()
        conc_vector = [conc_exs.get(specie, sym.Symbol(specie)) for specie in self.species]

        # If rates affected ConcEqs, we'd put that here.

        # This function imitates the format of get_evolution_function()
        concentration_func = sym.lambdify(
            args=(TIME, self.species_symbols),
            expr=conc_vector,
        )
        return concentration_func

    def get_evolution_function(self, rates: Optional[Dict] = None) -> Callable:
        """Gives the function which governs the evolution of the system.
        It will have signature (time, species). It closely corresponds to
        what is displayed by display_dynamics."""

        # This list is of the format {specie name: sympy expr} and may not
        #  cover self.species.
        ode_exs = self.get_ode_expressions()
        ddt_vector = [ode_exs.get(specie, sym.sympify(0)) for specie in self.species]

        # It is at this point that we substitute in the rate constants in
        #  our ddt_vector (also the conc_vector for good measure).
        # Luckily, get_rates gives it to us in a format ready for substitution,
        #  so long as self.reactions has not been shuffled recently.
        rates = rates or self.get_rates()
        ddt_vector = [ddt.subs(rates) for ddt in ddt_vector]

        # We make a function to be fed into SciPy's ODEINT solver.
        evolution_func_undecorated = sym.lambdify(
            # These arguments are sympy symbols, and determine the function's
            #  input signature. This function will have signature
            #  f(time, (specie1, specie2, ...) -> (specie1, specie2, ...)
            args=(TIME, self.species_symbols),
            # This vector of expressions gives the function its output shape.
            expr=ddt_vector,
        )

        # Get the concentration function, which has a similar signature,
        #  but it outputs concentrations, not first derivatives.
        concentration_func = self.get_concentration_function()

        # Now we must doctor the inputs of our evolution function so that
        #  it acts like our concentration function is being respected.
        def evolution_func(time: float, concs: List[float]):
            # Observe that conc_vector gives identity for all species that
            #  don't have a ConcEq.
            concs = concentration_func(time, concs)
            return evolution_func_undecorated(time, concs)
    ...
    def display_dynamics(self):
        """Display the expression governing the evolution of each species."""

        # --- Sympy manipulation functions ---
        def make_ddt(specie, expr):
            return sym.Eq(
                sym.Derivative(sym.Symbol(specie), TIME),
                expr,
                evaluate=False
            )

        def make_eq(specie, expr):
            return sym.Eq(sym.Symbol(specie), expr, evaluate=False)

        def make_zero(specie):
            return sym.Eq(sym.Symbol(specie), 0, evaluate=False)

        # --- Combine diffs and concs as sym.Eqs---
        diffs = self.get_ode_expressions()
        concs = self.get_conc_expressions()
        eqs = {}

        eqs.update({specie: make_ddt(specie, expr) for specie, expr in diffs.items()})
        eqs.update({specie: make_eq(specie, expr) for specie, expr in concs.items()})

        # --- Display with Jupyter ---
        # You could choose to iterate on (eq in eq.values()), but I want to
        #  display it in lock-step with self.species. This also has the
        #  effect of having extra species be visibly zero.
        for specie in self.species:
            eq = eqs.get(specie, make_zero(specie))
            latex = pretty_sym_subs(eq, fmt='[{}]')
            display.display(display.Latex(f'${latex}$'))

    display_ode_expressions = util.depreciate(display_dynamics)
