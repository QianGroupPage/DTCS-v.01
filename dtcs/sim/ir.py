...
from dtcs.spec.ir import IRSignature, IRSpecies
...
_SIGMA = 0.15 * np.sqrt(2) / (np.sqrt(2 * np.log(2)) * 2)
...
def simulate_ir(
    ...
    if deconvolute:
        ...
        deconv_cols = [(_DECONVOLUTED, specie) for specie in deconv_species]
        columns.extend(deconv_cols)

    df = _get_dataframe(x_range=x_range, columns=columns)
    ...
    if autoscale:
        ...
        scale_factor = _get_autoscale(sim_envelope=envelope, exp_envelope=exp_data)
    ...
    return df, scale_factor, deconv_concs
...
def _get_deconvolution(
    ...
    def envelope_from_concs(x_range, *concs: float):
        ...
        for index, specie in enumerate(species):
            hill = _get_gaussian(
                orbital=orbitals[str(specie)], x_range=x_range, conc=concs[index]
            )
...
def _get_gas_phase(
    xps, x_range: np.ndarray, exp_data: pd.Series, gas_interval: Tuple[float, float]
) -> np.ndarray:
...
def _get_x_range(
    orbitals: Dict[str, OrbitalInfo], species: List[SpeciesName]
) -> np.ndarray:
...
def _get_gaussian(orbital: OrbitalInfo, x_range: np.ndarray, conc: float) -> np.ndarray:
